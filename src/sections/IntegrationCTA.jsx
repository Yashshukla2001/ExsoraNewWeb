  ...
  return (
    <section className="relative overflow-hidden px-5 py-24 sm:px-8">
      <div className="mx-auto max-w-6xl">
        <div className="relative grid items-center gap-10 overflow-hidden rounded-3xl border border-white/8 bg-surface p-8 sm:p-14 lg:grid-cols-2">
          {/* starburst */}
          ...
          <Reveal variant="right" delay={0.1} className="grid grid-cols-3 gap-4">
            {ICONS.map((Icon, i) => (
              <motion.div
                key={i}
                animate={{ y: [0, i % 2 === 0 ? -8 : 8, 0] }}
                transition={{ duration: 4 + i, repeat: Infinity, ease: 'easeInOut' }}
                className="grid aspect-square place-items-center rounded-2xl border border-white/10 bg-white/[0.03] text-white shadow-[0_0_24px_rgba(255,193,94,0.08)]"
              >
