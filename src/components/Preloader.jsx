...
//           ].map((cls, i) => (
//             <motion.div key={i} className={`absolute h-6 w-6 border-accent-cyan/40 sm:h-10 sm:w-10 ${cls}`}
...
//          {/* content wrapper — keeps everything inside the viewport on small screens */}
//           <div className="relative z-10 mt-3 text-[10px] leading-relaxed tracking-[0.15em] text-muted uppercase sm:text-xs sm:tracking-[0.35em]">
...
//                 className="text-4xl font-black tracking-tighter xs:text-5xl sm:text-6xl"
//                 style={{
...
//           {/* progress bar */}
//          <motion.div className="relative z-10 mt-8 w-full max-w-[13rem] sm:mt-10"
    ...
    <AnimatePresence onExitComplete={onComplete}>
      {visible && (
        <motion.div key="preloader"
          exit={{ opacity: 0, filter: 'blur(20px)', scale: 1.06 }}
          transition={{ duration: 0.75, ease: [0.16, 1, 0.3, 1] }}
          className="preloader-shell fixed inset-0 z-[9999] flex flex-col items-center justify-center overflow-hidden bg-black"
        >
          {/* tech grid scan */}
          <div className="pointer-events-none absolute inset-0 [background-image:linear-gradient(rgba(90,209,255,0.04)_1px,transparent_1px),linear-gradient(90deg,rgba(90,209,255,0.04)_1px,transparent_1px)] [background-size:32px_32px]" />
          ...
          {/* ambient glow */}
          <motion.div className="pointer-events-none absolute h-56 w-56 rounded-full sm:h-72 sm:w-72"
            style={{ background: 'radial-gradient(circle,rgba(61,124,255,0.2),rgba(90,209,255,0.08),transparent 70%)', filter: 'blur(50px)' }}
            animate={{ scale: [0.9, 1.1, 0.9], opacity: [0.4, 0.7, 0.4] }}
            transition={{ duration: 3, repeat: Infinity, ease: 'easeInOut' }}
          />

          {/* content wrapper — keeps everything centered and inside the viewport on small screens */}
          <div className="relative z-10 mx-auto flex w-full max-w-xs flex-col items-center px-4 text-center sm:max-w-none sm:px-0">

            {/* wordmark — letters build one by one */}
            <div className="flex items-center justify-center gap-[1px] sm:gap-[2px]">
              {LETTERS.map((ch, i) => (
                <motion.span key={i}
                  initial={{ opacity: 0, y: 18, filter: 'blur(8px)' }}
                  animate={revealed > i ? { opacity: 1, y: 0, filter: 'blur(0px)' } : {}}
            ...
            {/* tagline */}
            <motion.p className="mt-3 text-[10px] leading-relaxed tracking-[0.15em] text-muted uppercase sm:text-xs sm:tracking-[0.35em]"
              initial={{ opacity: 0 }} animate={{ opacity: revealed >= LETTERS.length ? 1 : 0 }}
              transition={{ duration: 0.6 }}
            >
             CUSTOM BUSINESS SOLUTION || YOUR GROWTH PARTNER
            ...
            {/* progress bar */}
            <motion.div className="mt-8 w-full max-w-[13rem] sm:mt-10"
              initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }}>
              <div className="h-[2px] w-full overflow-hidden rounded-full bg-white/8">
                <motion.div animate={{ width: `${Math.min(progress, 100)}%` }}
                  transition={{ duration: 0.15, ease: 'easeOut' }}
